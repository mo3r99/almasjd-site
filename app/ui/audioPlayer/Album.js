  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <Link href={`/audio/${id}`}>
          <h3 className="text-base font-semibold text-foreground font-[family-name:var(--font-raleway)]">{title}</h3>
        </Link>
        <Link href={`/audio/${id}`}>
          <p className="text-sm text-muted-foreground truncate font-[family-name:var(--font-montserrat)]">{artist}</p>
        </Link>
      </div>
    </div>
  );
